...
def finding_fingerprint(finding: dict[str, Any], rules: dict[str, Any]) -> str:
    ...
    material = {field: selected(finding, field) for field in rule["fields"]}
    return digest_value(material, prefix=str(rule["prefix"]))
...
def active_ignore_rules(repository_root: Path) -> set[str]:
    ...
    try:
        ...
        raise FindingExportError("non-UTF-8 .gitignore cannot produce housekeeping Findings") from error
    return {
        line.strip()
...
def tracked_artifacts(repository_root: Path, names: set[str], suffixes: set[str]) -> list[str]:
...
def housekeeping_findings(
    ...
    *,
    ...
    if ".DS_Store" not in ignores or metadata:
        ...
        findings.append(
            make_finding(
                ...
                summary=detail,
                eligible=True,
                reason="Deterministic housekeeping fixer is allowlisted; tracked binary deletion remains review-only.",
                detected_at=detected_at,
                run_url=run_url,
                rules=rules,
            )
    ...
    if repository_has_python(repository_root):
        missing = [rule for rule in ("*.py[cod]", "__pycache__/") if rule not in ignores]
        caches = tracked_artifacts(repository_root, set(), {".pyc", ".pyo", ".pyd"})
        if missing or caches:
            detail = "Missing Python cache ignore rule(s): " + ", ".join(missing or ["none"])
            if caches:
...
def report_findings(
    ...
    *,
    ...
    for item in report.get("policy_findings", []):
        ...
        source_rule = str(item.get("rule") or "audit-policy")
        if source_rule == "actions-pin":
            rule_id = "missing-action-pin"
            eligible = True
            reason = "Action pin proposals remain review-only and require an approved immutable pin map."
        else:
            rule_id = source_rule if source_rule.replace("-", "").isalnum() else "audit-policy"
            eligible = False
            reason = "No deterministic Gardener fixer is approved for this audit rule."
        severity = "failure" if str(item.get("severity", "")).lower() == "error" else "warning"
        location = str(item.get("path") or "repository")
        findings.append(
            make_finding(
                repository=repository,
                rule_id=rule_id,
    ...
    for item in report.get("vulnerabilities", []):
        ...
        severity = "critical" if severity_value == "critical" else "failure" if severity_value == "high" else "warning"
        summary = (
            f"{item.get('vulnerability_id', 'unknown')} affects "
            f"{item.get('dependency', 'unknown')} {item.get('version', 'unknown')}."
        )
        findings.append(
            make_finding(
                repository=str(item["repo"]),
                ...
                category="security",
                severity=severity,
                location=str(item.get("source_file") or "repository"),
                summary=summary,
                eligible=False,
                reason="Dependency and lockfile changes are outside the initial automatic-remediation policy.",
                detected_at=detected_at,
                run_url=run_url,
                rules=rules,
            )
...
def coverage_repositories(coverage: dict[str, Any]) -> list[str]:
    ...
    if len(repositories) != 20 or len(set(repositories)) != 20:
        raise FindingExportError("Gardener coverage must contain 20 unique repositories")
    if any(not isinstance(value, str) or not value.startswith(f"{OWNER}/") for value in repositories):
        raise FindingExportError("Gardener coverage contains an invalid repository identity")
    return sorted(repositories)


def repository_snapshot(repository: str, repository_root: Path) -> dict[str, str]:
    commit = git(repository_root, "rev-parse", "HEAD")
    if len(commit) != 40 or any(value not in "0123456789abcdef" for value in commit):
        raise FindingExportError(f"invalid base commit for {repository}")
    remote_head = git(repository_root, "symbolic-ref", "refs/remotes/origin/HEAD")
    prefix = "refs/remotes/origin/"
    if not remote_head.startswith(prefix):
        raise FindingExportError(f"cannot resolve default branch for {repository}")
    branch = remote_head.removeprefix(prefix)
    if branch != "main":
        raise FindingExportError(f"unexpected default branch for {repository}: {branch}")
    return {"repository": repository, "base_branch": branch, "base_sha": commit}
...
def build_bundle(
    *,
    ...
    policy = load_object(infra_root / "policy/gardener-automation.json", "Gardener automation policy")
    coverage = load_object(infra_root / "policy/gardener-github-app-coverage.json", "Gardener coverage policy")
    rules = load_object(infra_root / "contracts/v1/fingerprint-rules.json", "fingerprint rules")
    finding_schema = load_object(infra_root / "contracts/v1/finding.schema.json", "Finding schema")
    contract_module = load_contract_module(infra_root)
    repositories = coverage_repositories(coverage)
    covered = set(repositories)
    detected_at = generated_at.astimezone(timezone.utc).isoformat(timespec="seconds").replace("+00:00", "Z")
    ...
    for repository in repositories:
        ...
        if not (root / ".git").is_dir():
            findings.append(
                make_finding(
                    repository=repository,
                    rule_id="audit-checkout-unavailable",
                    check_id="audit-checkout-unavailable",
                    category="evidence",
                    severity="failure",
                    location="repository",
                    summary="The scheduled audit did not retain an authenticated checkout for this covered repository.",
                    eligible=False,
                    reason="Incomplete repository evidence fails closed and cannot be remediated.",
                    detected_at=detected_at,
                    run_url=run_url,
                    rules=rules,
                )
    ...
    for finding in findings:
        ...
        if errors:
            raise FindingExportError(f"canonical Finding failed validation: {errors[0]}")
    ...
    bundle: dict[str, Any] = {
        ...
        "expires_at": (generated_at + timedelta(hours=int(policy["finding_bundle"]["maximum_age_hours"])))
        .astimezone(timezone.utc)
        .isoformat(timespec="seconds")
        .replace("+00:00", "Z"),
        "public_only": True,
        "source_report_digest": digest_file(report_path),
        "repository_snapshots": sorted(snapshots, key=lambda value: value["repository"]),
...
def main() -> int:
    ...
    parser.add_argument("--source-run-id", default=os.getenv("GITHUB_RUN_ID", "local"))
    parser.add_argument("--source-run-attempt", type=int, default=int(os.getenv("GITHUB_RUN_ATTEMPT", "1")))
    ...
    try:
        ...
        if not str(args.source_run_id).isdigit() or str(args.source_run_id) == "0":
        ...
        if len(args.source_commit) != 40 or any(value not in "0123456789abcdef" for value in args.source_commit):
            raise FindingExportError("source commit must be a lowercase 40-character SHA")
        bundle = build_bundle(
